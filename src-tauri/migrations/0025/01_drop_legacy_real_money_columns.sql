-- DAT-1.e (#55): drop the legacy REAL money columns now that every read and
-- write path is on the *_cents (INTEGER minor units) columns added in 0015
-- and repointed in DAT-1.b/c/d (PRs #117/#122/#130).
--
-- REQUIRES SQLite >= 3.35 for `ALTER TABLE ... DROP COLUMN`. The bundled
-- `tauri-plugin-sql` / `sqlx` runtime ships with SQLite well above 3.35.
--
...
-- ------------------------------
-- IEEE-754 REAL cannot represent decimals like 0.10 exactly, so totals built
-- from these columns drift and reconciliations against bank statements fail.
-- The integer-cent columns added in 0015 hold the source of truth; keeping
-- the REAL columns around invites accidental reads of stale or zeroed values
-- (createInvoice / createInvoiceItem already write only the *_cents columns;
-- a separate follow-up will retire the remaining transitional writes in
-- createIncomingInvoice / createPayment / cancelInvoice).
--
-- Mechanics
-- ---------
-- SQLite ALTER TABLE ... DROP COLUMN refuses to drop a column that appears
-- in any CHECK constraint, FK, generated column, index, or trigger
-- expression. So:
--   1. Drop the audit triggers (0019) and the invoice immutability trigger
--      (0020/0021), all of which name the legacy columns.
--   2. For invoices the legacy columns have no CHECK constraints (they were
--      never CHECKed in 0001 / 0016). Plain ALTER TABLE DROP COLUMN works.
--   3. For invoice_items the legacy columns also have no CHECK constraints
--      (the >= 0 CHECKs were dropped in the 0021 storno rebuild). Plain
--      ALTER TABLE DROP COLUMN works.
--   4. For payments the legacy `amount` column IS referenced by a CHECK
--      constraint (payments_amount_check from 0001 / 0011). SQLite cannot
--      drop a CHECK in place, so we use the standard CREATE _new / INSERT
--      SELECT / DROP / RENAME recipe to rebuild the table without the
--      legacy column or its CHECK. The semantic of the CHECK (amount > 0)
--      is preserved on the surviving `amount_cents` column.
--   5. For incoming_invoices the legacy columns ARE referenced by inline
--      CHECK constraints (>= 0 CHECKs from 0007). Same table-rebuild
--      recipe. The rebuilt table also preserves `s3_key` (0009/0013) and
--      `local_path` (0022); `file_data` was dropped in 0023 (note: 0023
--      may not be registered in lib.rs on this branch, so the rebuild
--      defensively omits `file_data` from the SELECT either way).
--   6. Recreate the immutability and audit triggers without any reference
--      to the dropped columns. Their semantics remain unchanged for the
--      surviving columns; the dropped fields simply no longer appear in
--      audit diffs.
--
-- The down migration in 0025_down recreates the REAL columns and backfills
-- them from *_cents / 100.0. The textual schema cannot round-trip exactly
-- (column order differs, trigger text differs, backfill is cent-rounded),
...
-- 2. invoices: simple DROP COLUMN (no CHECK constraints reference these).
...
-- 3. invoice_items: simple DROP COLUMN (the >= 0 CHECKs were removed in 0021).
ALTER TABLE invoice_items DROP COLUMN unit_price_net;
ALTER TABLE invoice_items DROP COLUMN line_total_net;

-- 4. payments: rebuild to drop `amount` (REAL) and its CHECK (amount > 0).
--    The "must be > 0" semantic is preserved on the cents column via a
--    renamed CHECK constraint on amount_cents.
...
-- 5. incoming_invoices: rebuild to drop net_amount/tax_amount/gross_amount
--    and their inline CHECK (>= 0) constraints from 0007. The cents
--    columns added in 0015 had no CHECK constraints; we preserve that
--    (no new constraints introduced on the cents columns). The
--    `local_path` column added in 0022 and the `s3_key` column added in
--    0009 are both preserved; `file_data` is intentionally omitted from
--    the new table and the SELECT (it was dropped in 0023, but if 0023
--    has not been applied at runtime, this rebuild drops it as a side
--    effect — see the migration comment block at the top).
...
INSERT INTO incoming_invoices_new (
  id, company_id, supplier_id, invoice_number, invoice_date, status,
  file_name, file_type, notes, created_at, updated_at, s3_key,
  net_cents, tax_cents, gross_cents, local_path
)
SELECT
  id, company_id, supplier_id, invoice_number, invoice_date, status,
  file_name, file_type, notes, created_at, updated_at, s3_key,
...
-- 6. Recreate the invoice immutability trigger (post-0021 form, including
--    references_invoice_id and cancellation_reason) without the legacy
--    money columns. Status / updated_at / s3_key remain mutable on issued
--    invoices by virtue of being absent from the WHEN clause, matching
--    the original 0020/0021 semantics.
...
-- 7. Recreate the audit triggers (from 0019) without legacy money column
--    fields. The structure and JSON output shape are otherwise identical;
--    consumers of invoice_audit simply stop seeing the dropped fields.

-- invoices audit triggers ---------------------------------------------------
