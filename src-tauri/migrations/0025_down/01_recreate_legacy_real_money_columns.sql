-- DAT-1.e (#55) down migration: best-effort restore of the legacy REAL money
-- columns dropped in 0025.
--
-- REQUIRES SQLite >= 3.35 for the up's `ALTER TABLE ... DROP COLUMN`. The
-- down uses `ALTER TABLE ... ADD COLUMN` and table rebuilds, both available
-- on every SQLite version this project supports.
--
-- Limitations (why this is marked `.noop_down` in the up directory)
-- -----------------------------------------------------------------
-- * Column order: SQLite ADD COLUMN appends to the end, so the restored REAL
--   columns appear after all the INTEGER cents columns rather than in their
--   original positions. The textual CREATE TABLE therefore differs from the
--   pre-0025 schema even though the data is semantically equivalent.
-- * Precision: the original REAL values had IEEE-754 representations
--   (already lossy for many decimals); the restored values are
--   `ROUND(cents / 100.0, 2)`. For rows authored under the cents regime the
--   restored value is exactly cent-rounded. For rows authored under the
--   REAL regime and migrated through 0015 the original sub-cent IEEE-754
--   noise is gone — we deliberately discard it because it was never the
--   source of truth.
-- * Triggers: the audit and immutability triggers are reinstated in their
--   post-0021 forms (which reference the legacy columns). Audit log rows
--   written during the 0025 era will not contain those fields.
--
-- Because the schema does not round-trip exactly, this down migration is
-- marked `.noop_down` in the 0025/ directory. The TEST-2.a round-trip
...
-- 2. invoices: restore the three REAL columns and backfill from cents.
...
UPDATE invoices SET
  ...
  tax_amount   = ROUND(tax_cents   / 100.0, 2),
  gross_amount = ROUND(gross_cents / 100.0, 2);

-- 3. invoice_items: restore the two REAL columns and backfill from cents.
...
UPDATE invoice_items SET
  unit_price_net = ROUND(unit_price_net_cents / 100.0, 2),
  line_total_net = ROUND(line_total_net_cents / 100.0, 2);

-- 4. payments: rebuild to add `amount` (REAL) back with its CHECK constraint.
--    The 0025 up rewrote the CHECK to operate on `amount_cents`; we restore
--    the original `payments_amount_check CHECK (amount > 0)` here.
...
SELECT
  ...
  method, reference, note, created_at, updated_at, amount_cents
FROM payments;
...
-- 5. incoming_invoices: rebuild to add legacy REAL columns back with their
--    inline CHECK (>= 0) constraints from 0007.
...
INSERT INTO incoming_invoices_new (
  ...
  status, file_name, file_type, notes, created_at, updated_at, s3_key,
  net_cents, tax_cents, gross_cents, local_path
)
SELECT
  id, company_id, supplier_id, invoice_number, invoice_date,
  ROUND(net_cents   / 100.0, 2),
  ROUND(tax_cents   / 100.0, 2),
  ROUND(gross_cents / 100.0, 2),
  status, file_name, file_type, notes, created_at, updated_at, s3_key,
  net_cents, tax_cents, gross_cents, local_path
...
-- 6. Recreate the invoice immutability trigger from migration 0021 (the
--    post-storno version that includes references_invoice_id and
--    cancellation_reason in the WHEN clause, and the legacy money columns).
...
-- 7. Recreate the audit triggers from migration 0019 (the original versions
--    that include the legacy REAL money columns in their JSON output).
