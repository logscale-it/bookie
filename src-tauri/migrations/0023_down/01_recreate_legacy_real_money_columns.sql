...
--   pre-0023 schema even though the data is semantically equivalent.
-- * Precision loss: backfilling from ROUND(cents / 100.0) is not lossless —
--   the original REAL values had IEEE-754 representation; the restored values
--   are the cent-rounded equivalents.
-- * Triggers: the audit and immutability triggers are reinstated in their
--   post-0021 forms (which reference the legacy columns) but the audit log
--   rows written during the 0023 era will not contain those fields.
--
-- Because of these limitations this down migration is marked .noop_down in
-- the 0023/ directory; the TEST-2.a round-trip harness logs a warning for
-- schema divergence rather than failing the test.

PRAGMA foreign_keys=OFF;
BEGIN;

-- 1. Restore legacy REAL columns (appended to end of each table).
...
-- 2. payments: rebuild to add `amount` back with its CHECK constraint.
...
-- 3. incoming_invoices: rebuild to add legacy REAL columns back with their
-- CHECK constraints.
...
INSERT INTO incoming_invoices_new (
  ...
  status, file_data, file_name, file_type, notes, created_at, updated_at, s3_key,
...
-- 4. Reinstate the immutability trigger (post-0021 form, with legacy columns).
...
-- 5. Reinstate the audit triggers (post-0019 form, with legacy columns).
