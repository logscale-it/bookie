...
--   2. For invoices and invoice_items the legacy columns have no CHECK in
--      the current schema (invoice_items's >=0 CHECKs were dropped in the
--      0021 storno rebuild; invoices.*_amount were never CHECKed). Plain
--      ALTER TABLE DROP COLUMN works.
--   3. For payments and incoming_invoices the legacy columns ARE referenced
--      by CHECK constraints (payments_amount_check from 0011, the inline
--      CHECKs from 0007). SQLite cannot drop those CHECKs in place, so we
--      use the standard CREATE _new / INSERT SELECT / DROP / RENAME recipe
--      to rebuild each table without the legacy column or its CHECK. The
--      semantic of the CHECK (amount > 0 / >= 0) is preserved on the
--      surviving cents columns so we don't silently weaken the invariant.
--   4. Recreate the immutability and audit triggers without any reference
--      to the dropped columns. Their semantics (which columns are protected
--      / audited) are unchanged for everything that survived; we simply
--      stop emitting and comparing the now-nonexistent legacy fields.
--
-- The down migration in 0022_down recreates the REAL columns and backfills
-- them from the *_cents columns via value/100.0, then reinstates the
-- triggers as they were after migration 0021. Round-tripping the two
-- migrations on a populated DB therefore lands back on a schema that is
-- byte-compatible with the post-0021 state.
...
-- The "must be > 0" semantic is preserved on the cents column so we don't
-- silently weaken the constraint when the legacy column goes away.
...
-- preserved on the cents columns so we don't silently weaken what callers
-- relied on.
...
-- 6. Recreate the invoice immutability trigger (originally from 0021) without
-- the legacy money columns. Status / updated_at / s3_key remain mutable on
...
-- 7. Recreate the audit triggers (originally from 0019) without legacy
-- money column fields. The structure / output JSON shape is otherwise
-- identical; consumers of invoice_audit just stop seeing the dropped fields.
