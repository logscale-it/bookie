...
-- (createInvoice / createIncomingInvoice / createInvoiceItem already write
-- only the *_cents columns, so the legacy columns are 0 on every new row
-- and the old historical values are no longer kept in sync).
--
-- Mechanics
-- ---------
-- SQLite ALTER TABLE ... DROP COLUMN requires SQLite >= 3.35 and refuses to
-- drop a column that appears in any CHECK constraint, FK, generated column,
-- index, or trigger expression. So:
...
--      never CHECKed). Plain ALTER TABLE DROP COLUMN works.
...
--      constraint (payments_amount_check from 0011). SQLite cannot drop a
--      CHECK in place, so we use the standard CREATE _new / INSERT SELECT /
--      DROP / RENAME recipe to rebuild the table without the legacy column
--      or its CHECK. The semantic of the CHECK (amount > 0) is preserved on
--      the surviving `amount_cents > 0` column.
--   5. For incoming_invoices the legacy columns ARE referenced by CHECK
--      constraints (inline >= 0 CHECKs from 0007). Same table-rebuild recipe.
--      The rebuilt table also includes `local_path` (added in 0022).
--   6. Recreate the immutability and audit triggers without any reference to
--      the dropped columns. Their semantics remain unchanged for surviving
--      columns; the dropped fields simply no longer appear in audit diffs.
--
-- The down migration in 0023_down recreates the REAL columns and backfills
-- them from *_cents / 100.0. The textual schema cannot round-trip exactly
-- (column order differs, precision of backfill vs original), so a
-- .noop_down marker is present in the 0023/ directory and the round-trip
...
-- 3. invoice_items: simple DROP COLUMN (CHECKs were removed in 0021).
...
-- The "must be > 0" semantic is preserved on the cents column.
...
-- and their CHECK (>= 0) constraints from 0007. The >= 0 semantic is
-- preserved on the cents columns. The `local_path` column added in 0022 is
-- preserved in the rebuilt table.
...
-- 6. Recreate the invoice immutability trigger (from 0021) without the
-- legacy money columns. Status / updated_at / s3_key remain mutable on
-- issued invoices by virtue of being absent from the WHEN clause, matching
