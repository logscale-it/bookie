...
// dropped from the schema in migration 0023.
//
// `gross_cents` is derived from net_cents + tax_cents at write time.
// `local_path` is populated only by the backfill script (DAT-5.a), never
// by the create path; the column defaults to NULL at the SQL layer.
type CreateIncomingInvoice = Omit<
  IncomingInvoice,
  "id" | "created_at" | "updated_at" | "gross_cents" | "local_path"
>;
